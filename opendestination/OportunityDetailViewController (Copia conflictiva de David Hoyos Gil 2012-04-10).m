...
- (void)viewDidLoad
{
  ...
  //[self.backButton setHidden:NO];
    ...
    _ownerImage.layer.cornerRadius = 10.0;
    _ownerImage.layer.borderWidth = 2.0;
  ...
  if ( self.opportunity ) [self reload];
   ...
   // [notInterestedButton setTitle:NSLocalizedString(@"notInterestedBtnKey", @"NotInterested") forState:UIControlStateNormal];
    ...
    if (_opportunity.title.length > 18) {
    CGRect currentFrame = titleLabel.frame;
        
    CGSize max = CGSizeMake(titleLabel.frame.size.width, titleLabel.frame.size.height);
    CGSize expected = [_opportunity.title sizeWithFont:[UIFont systemFontOfSize:18.0f] constrainedToSize:max lineBreakMode:titleLabel.lineBreakMode]; 
...
- (void) reload
{
  ...
  NSString * desc = (realname.length>0 ?[_opportunity.owner objectForKey:@"real_name"]:[_opportunity.owner objectForKey:@"user_name"]);
