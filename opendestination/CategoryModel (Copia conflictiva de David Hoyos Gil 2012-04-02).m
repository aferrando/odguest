...
{
  if ( [user signedIn] ) {
    NSString * url = [NSString stringWithFormat:@"%@/category/get?destination_id=%d&user_id=%d&uuid=%@&category_id=%d",[[Destination sharedInstance] destinationService], [user destinationID], [user userID], [user udid], categoryID];
...
- (void) parseDataDict:(NSDictionary *)dict
{
  ...
  if (( [[_userInfo objectForKey:@"numOportunitys"] isKindOfClass:[NSNumber class]] ) || ( [[_userInfo objectForKey:@"numOportunitys"] isKindOfClass:[NSString class]] ) )
    self.numOpportunities = [[_userInfo objectForKey:@"numOportunitys"] integerValue];
