...
@synthesize points, goal, opportunities, tags, destinationName;
...
#warning the Locale is hardcode to en_US
    ...
    self.birthDate = [[[NSDate alloc] init] autorelease];
...
{
  [self saveSettings];
  [_locationManager release], nil;
  [_myLocation release], nil;
  [dataDict release];
  [_userName release];
  [_password release];
  [_image release];
...
- (void) setPassword:(NSString *)password
{
    @synchronized(self) {
      ...
      _password = nil;
    if ( password )
      _password = [[Utilities returnMD5Hash:password] retain];
  }
}
...
- (void) deviceRegister
{
  ...
  if ( _realName )
    url = [NSString stringWithFormat:@"%@&realname=%@",url,_realName];
  if ( _gender )
...
- (void) postImage:(UIImage *)image
{
  if (!postingData && (image) )
  {
    ...
    [_sendingImage release];
    _sendingImage = nil;
    _sendingImage = [image retain];
...
- (void) setParam:(id)property
{
  ...
  if ( property == _realName )
    fullURLString = [NSString stringWithFormat:@"%@&realname=%@",url,_realName];
...
-(void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation{
  ...
  if ( _myLocation == nil || oldLocation.horizontalAccuracy > newLocation.horizontalAccuracy) {
    [_myLocation release];
    _myLocation = nil;
    _myLocation = [newLocation retain];
#ifdef __DEBUG__
...
{
  if (!string) string = @"Connection failed with a remote error.";
  [[[[UIAlertView alloc] initWithTitle:@"Error"
                             message:string
                            delegate:self
                   cancelButtonTitle:@"Cancel"
                   otherButtonTitles:nil] autorelease] show]; //TODO: Language;
}
      ...
      if ([jsonObject isKindOfClass:[NSDictionary class]]) { 
        ...
        else
        {
          ...
          if ( [(NSNumber *)[dict objectForKey:@"errorCode"] integerValue] == 1003)
            [self showAlert:@"User name already exists"];
          else
            [self showAlert:nil];
...
-(void)requestDidFinishWithResponse:(NSData *)responseData {
  ...
  switch(error)
    ...
    case 0:
      [_sendingImage release];
      _sendingImage = nil;
      alert = [[[UIAlertView alloc]initWithTitle:@"Profile updated"
                                           message:nil
                                          delegate:self cancelButtonTitle:@"OK"
                                 otherButtonTitles: nil] autorelease];
      break;
    default:
      alert = [[[UIAlertView alloc]initWithTitle:@"Update Failed"
                                         message:nil
                                        delegate:self
                               cancelButtonTitle:@"Cancel"
                               otherButtonTitles:@"Send again", nil] autorelease];
      break;
  }
