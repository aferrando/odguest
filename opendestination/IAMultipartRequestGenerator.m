...
void requestCallback (CFReadStreamRef readStream, CFStreamEventType event, void *data) {
    ...
    switch (event) {
        ...
        case kCFStreamEventErrorOccurred: {
            ...
            NSError *streamError = [NSError errorWithDomain:@"Error occurred on event" code:error.error userInfo:NULL];
...
//Esta función ha sido creada por Marcos Alba. Es una modificación de la anterior para que el servidor reconozca imágenes png o jpeg
-(void)setData:(NSData *)data forField:(NSString *)fieldName type:(int)pngOrJpeg fileName:(NSString*)fn{
    
    NSMutableString *fieldString = [NSMutableString string];
    
    if(pngOrJpeg==0){//this is jpeg
        [fieldString appendFormat:@"Content-Disposition: form-data; name=\"%@\"; filename=\"%@.jpeg\"\r\n", fieldName,fn];
        [fieldString appendString:@"Content-Type: image/jpeg\r\n\r\n"];
    }
    else{
        [fieldString appendFormat:@"Content-Disposition: form-data; name=\"%@\"; filename=\"%@.png\"\r\n", fieldName,fn];
        [fieldString appendString:@"Content-Type: image/png\r\n\r\n"];
    }
    [requestData appendData:[fieldString dataUsingEncoding:NSUTF8StringEncoding]];
    [requestData appendData:data];
    [requestData appendData:[[NSString stringWithString:@"\r\n"] dataUsingEncoding:NSUTF8StringEncoding]];
...
//esta linea ha sido modificada por David Hoyos para que se ejecute la respuesta en el mainThread
    [(NSObject*) delegate performSelectorOnMainThread:@selector(requestDidFinishWithResponse:) withObject:data waitUntilDone:YES]; //[delegate requestDidFinishWithResponse:data];
