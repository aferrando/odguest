...
- (void) reload
{
  ...
  if ( [user signedIn] ) {
    NSString * url = [NSString stringWithFormat:@"%@/category/get?destination_id=%d&user_id=%d&uuid=%@&category_id=%d&start_date=%@&end_date=%@",[[Destination sharedInstance] destinationService], [user destinationID], [user userID], [user udid], categoryID, [dateFormatter stringFromDate:today], [dateFormatter stringFromDate:endate]];
      if (categoryID==0) {
